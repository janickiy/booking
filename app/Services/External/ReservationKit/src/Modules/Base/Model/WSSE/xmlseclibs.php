...
function C14NGeneral($element, $exclusive=FALSE, $withcomments=FALSE)
{
	...
	//if (($php_version[0] > 5) || ($php_version[0] == 5 && $php_version[1] >= 2)) todo phalanger fix
	if (0 && (($php_version[0] > 5) || ($php_version[0] == 5 && $php_version[1] >= 2)))
	{
...
class XMLSecurityKey
{
	...
	public function loadKey($key, $isFile=FALSE, $isCert = FALSE)
	{
		...
		if ($this->cryptParams['library'] == 'openssl')
		{
			...
			{
				//$this->key = openssl_get_privatekey($this->key, $this->passphrase); todo phalanger fix
				$this->key = openssl_get_privatekey($this->key);
			}
		}
  ...
  <ds:SignedInfo>
	...
	public function locateSignature($objDoc)
	{
		...
		if ($doc)
		{
			...
			//$nodeset = $xpath->query($query, $objDoc); // todo phalanger fix
	...
	private function canonicalizeData($node, $canonicalmethod)
	{
		...
		$php_version = explode('.', PHP_VERSION);
		//if (($php_version[0] < 5) || ($php_version[0] == 5 && $php_version[1] < 2)) todo phalanger fix
		/*if (1 || ($php_version[0] < 5) || ($php_version[0] == 5 && $php_version[1] < 2))
		{
