...
namespace App\Http\Controllers\TranslationManager;

use App\Http\Controllers\TranslationManager\Events\TranslationsExportedEvent;
...
class Manager
{
     ...
     * @return bool
     */
    public function isExistTranslation($key, $locale, $group)
    {
        $result = TManager::where('locale',$locale)->where('group',$group)->where('key',$key)->first();
    ...
    public function importTranslation( $key, $value, $locale, $group, $replace = false )
    {
        ...
        $value       = (string) $value;
        $translation = TManager::firstOrNew( [
            'locale' => $locale,
            'group'  => $group,
            'key'    => $key,
        ] );
    ...
    public function exportTranslations( $group = null, $json = false )
    {
        ...
        if ( !is_null( $group ) && !$json ) {
            if ( !in_array( $group, $this->config[ 'exclude_groups' ] ) ) {
                ...
                $tree = $this->makeTree( TManager::ofTranslatedGroup( $group )
        ...
        if ( $json ) {
            $tree = $this->makeTree( TManager::ofTranslatedGroup( self::JSON_GROUP )
                ->orderByGroupKeys( array_get( $this->config, 'sort_keys', false ) )
                ->get(), true );
    ...
    public function transDB($key = '')
    {
        if (!empty($key)) {
            ...
            $trans = TManager::where('group', $group)->where('key', $key)->where('locale', config('app.locale'));

            if ($trans->count() == 0) {
                $list = $this->getTranslations($group);
     ...
     * @return mixed
     */
    public function getTranslations($group = null)
    {
        $tree = $this->makeTree( TManager::ofTranslatedGroup( $group )
            ->orderByGroupKeys( array_get( $this->config, 'sort_keys', false ) )
            ->get() );

        return  $group ? $tree[config('app.locale')][$group] : $tree[config('app.locale')];
    }
    ...
    {
        $groups = TManager::whereNotNull( 'value' )->selectDistinctGroup()->get( 'group' );
    ...
    {
        TManager::whereNull( 'value' )->delete();
    }
    ...
    public function getLocales()
    {
        if ( empty( $this->locales ) ) {
            $locales = array_merge( [ config( 'app.locale' ) ],
                TManager::groupBy( 'locale' )->pluck( 'locale' )->toArray() );
    ...
    public function removeLocale( $locale )
    {
        ...
        TManager::where( 'locale', $locale )->delete();
    }
